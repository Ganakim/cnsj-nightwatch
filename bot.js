...
function nightwatchToggle(){
    var now = moment()
    console.log(now.isBefore(times.close) ? `${moment(now.format('ddd'))}${times[now.isBefore(times.open) ? 'open' : 'close'].format('ha')}` : `${times.days[times.days.indexOf(now.format('ddd'))+1]}${times.open.format('ha')}`)
